...
| **Provenance** | Record of where a data value came from. In KYC, every identity field traces to one of three sources — OCR read, user-confirmed, or externally verified — kept separate so corrections never destroy evidence. | `05` C-02a |
| **Bootstrap (self-bootstrap)** | The user's first back-office call (`POST /kyc/self/bootstrap`): creates-or-resumes their applicant plus open attempt and binds their login (session + identity link). Idempotent — reopening resumes instead of duplicating. | `05` C-01, `03 §5.1` |
